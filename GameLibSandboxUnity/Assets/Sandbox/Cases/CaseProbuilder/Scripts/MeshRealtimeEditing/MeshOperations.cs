...
public static class MeshOperations
{
    ...
    public static Face[] ExtrudeFaces(this ProBuilderMesh mesh, ExtrudeMethod method, float distance, params Face[] faces)
    {
        var newFaces = mesh.Extrude(faces, method, distance);
        mesh.ToMesh(MeshTopology.Triangles);
        mesh.Refresh();
        return newFaces;
    }

    public static Face[] ExtrudeFaces(this ProBuilderMesh mesh, ExtrudeMethod method, float distance, params int[] faces)
    {
        var Faces_formatted = faces.Select((int item) => mesh.faces[item]);
        var newFaces = mesh.Extrude(Faces_formatted, method, distance);
        mesh.ToMesh(MeshTopology.Triangles);
        mesh.Refresh();
        return newFaces;
    }
    ...
    public static void MoveVertices(this ProBuilderMesh mesh, Vector3 movement, params int[] vertices)
    {
        ...
        mesh.TranslateVertices(vertices, movement);
        mesh.ToMesh(MeshTopology.Triangles);
        mesh.Refresh();
    }
    ...
    public static void ScaleFaces(this ProBuilderMesh mesh, Vector3 origin, float scaleFactor, params Face[] faces)
    {
        List<int> vertices = new List<int>();
