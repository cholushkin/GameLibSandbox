...
{
    [CreateAssetMenu(fileName = "TilesetGenerator", menuName = "GameLib/ScriptableObjects/TilesetGenerator", order = 1)]
    public class TilesetGenerator : ScriptableObject
    {
        ...
        public int PaletteIndex; // todo: replace on ColorPalette
        public string OutputDirectory;

        [Button()]
        public void CreateDebugBitmap()
        {
            if (Type == TilesetType.MultiscaleTruchet)
            {
                var tileset = new TruchetTilesetDrawer(TileSize, DivisionLevels, Palette.PaletteList[PaletteIndex]);
                var outputFilePath = Path.Combine(Application.dataPath, OutputDirectory, TilesetName, $"Dbg_{TilesetName}.png");
                Directory.CreateDirectory(Path.GetDirectoryName(outputFilePath));
                tileset.GenerateDebugImage(outputFilePath);
            }
        ...
        public void CreateTileset()
        {
            if (Type == TilesetType.MultiscaleTruchet)
            {
                var tilesetDrawer = new TruchetTilesetDrawer(TileSize, DivisionLevels, Palette.PaletteList[PaletteIndex]);
                Directory.CreateDirectory(Path.Combine(Application.dataPath, OutputDirectory, TilesetName));
                var tileset = ScriptableObjectUtility.CreateAsset<Tileset>(Path.Combine("Assets", OutputDirectory, TilesetName), $"{TilesetName}.asset");
                tilesetDrawer.SaveTiles(Application.dataPath, OutputDirectory, TilesetName, tileset);
            }
            AssetDatabase.Refresh();
        }
    }

    class TruchetTilesetDrawer
    {
        readonly int levels;
        ...
        public TruchetTilesetDrawer(int tileSize, int levels, Palette palette)
        {
        ...
        public void GenerateDebugImage(string filePath)
        {
        ...
        public void SaveTiles(string appDataPath, string outputPath, string tilesetName, Tileset tileset)
        {
            tileset.Divisions = new Tileset.Division[levels];
            for (int i = 0; i < levels; i++)
            {
                tileset.Divisions[i] = new Tileset.Division();
                tileset.Divisions[i].Tiles = new Texture2D[tileCount];
                for (int j = 0; j < tileCount; j++)
                {
                    tileArray[i, j].Save(Path.Combine(appDataPath, outputPath, tilesetName, $"d{i}t{j}.png"), ImageFormat.Png);
                    AssetDatabase.Refresh();
                    var tileTex = AssetDatabase.LoadAssetAtPath<Texture2D>(Path.Combine("Assets", outputPath, tilesetName, $"d{i}t{j}.png"));
                    
                    // Load the asset importer
                    TextureImporter importer = AssetImporter.GetAtPath(Path.Combine("Assets", outputPath, tilesetName, $"d{i}t{j}.png")) as TextureImporter;
                    ...
                    Assert.IsNotNull(tileTex);
                    tileset.Divisions[i].Tiles[j] = tileTex;
                }
            }
        }
